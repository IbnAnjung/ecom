...
type SellerAuthDataResponse struct {
	ID   int64  `json:"id"`
...
type SellerAuthResponse struct {
	...
	Data    SellerAuthDataResponse `json:"data"`
}

func SellerAuthenticationMiddleware(authServiceBaseURI string) echo.MiddlewareFunc {
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			req, err := http.NewRequestWithContext(c.Request().Context(), "PATCH", fmt.Sprintf("%s/validate_seller", authServiceBaseURI), bytes.NewReader(jsonBody))
			if err != nil {
			...
			c.Set(pkghttp.SellerUserIdContextKey, resBody.Data.ID)
